  ...
  static login(callback) {
    ...
    function (error, response, body) {
      ...
      if ( !error && ( response && response.statusCode < 400 ) && ( token = response.headers.token ? response.headers.token : false ) ) {
        ...
        logger.error("Error logging in to API at " + (config.get('mattermost.CHAT_INTERNAL_URL') + config.get('mattermost.API_PATH') + "/users/login") + ": " + error + ", status code: " + ( response && response.statusCode ? response.statusCode : "" ) + ", body: " + ( body && typeof response.body === 'object' ? JSON.stringify(body) : body ) + ", token: " + token);
        callback(null);
