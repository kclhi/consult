...
class Provenance {
  ...
  static httpRequest(method, action, params, callback) {
    ...
    const URL = config.PROVENANCE_SERVER_URL + "/" + action;
    ...
    request({
      ...
      if ( error || (response && response.statusCode >= 400) ) {
        ...
        console.log("Cannot contact provenance server. " + error + " " + ( ( body && typeof body === "object" ) ? JSON.stringify(body) : body ) + " " + ( response.statusCode ? response.statusCode : "Status unknown." ));
        callback({});
