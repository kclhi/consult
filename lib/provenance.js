  ...
  static httpRequest(method, action, params, callback) {
    ...
    request({
      ...
      if ( error || (response && response.statusCode >= 400) ) {
        ...
        console.error("Cannot contact provenance server. " + error + " " + ( ( body && typeof body === "object" ) ? JSON.stringify(body) : body ) + " " + ( response && response.statusCode ? response.statusCode : "Status unknown." ));
        callback({});
