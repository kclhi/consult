  ...
  static callFHIRServer(query, params, callback) {
    ...
    request.get(url, {
      ...
      if ( error || (response && response.statusCode >= 400) ) {
        ...
        logger.error("Cannot contact FHIR server. " + error + " " + ( ( body && typeof body === "object" ) ? JSON.stringify(body) : body ) + " " + ( response && response.statusCode ? response.statusCode : "Status unknown." ));
        callback({});
  ...
  static validPath(object, path) {
    ...
    for (var child in path) {
      ...
      if ( object[path[child]] ) {
        ...
        logger.error("Valid path check error. " + path[child] + " not in " + object);
        return false
