  ...
  static callFHIRServer(url, method, requestBody, callback, username="", password="") {
    ...
    function (error, response, body) {
      ...
      if (!error && response.statusCode <= 201) {
        ...
        console.log("Error when contacting FHIR server: " + JSON.stringify(requestBody).substring(0, 300) + " " + error + " " + ( ( body && typeof body === "object" ) ? JSON.stringify(body) : body ) + " " + ( response && response.statusCode ? response.statusCode : "Status unknown." ));
        callback(400);
