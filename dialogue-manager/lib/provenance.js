  ...
  static httpRequest(method, action, params, port, callback) {
    ...
    request({
      ...
      if ( error || (response && response.statusCode >= 400) ) {
        ...
        logger.error("Cannot contact provenance server for: " + action + ". Error: " + error + ". Body: " + ( ( body && typeof body === "object" ) ? JSON.stringify(body) : body ) + ". Status: " + ( response && response.statusCode ? response.statusCode : "Status unknown." ));
        callback({});
